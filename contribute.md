...
4. 著作权说明：文章著作权归原作者所有，除非另有声明，所有权利保留。本刊不会代表作者授予内容使用许可。
...
* 请勿植入宣传内容。
* 如稿件含 AI 生成内容（哪怕只有部分章节），请在文章 front matter 中标注 `ai-generated: true`，页脚将展示「含AI生成内容」标识；未标注的文章默认视为人工撰写，并在文章底部展示 Not By AI（由真人撰写）徽章。
* 内容使用许可是可选项，默认不启用。如您希望以特定许可协议授权他人使用稿件，请在 front matter 中声明 `license` 字段（如 `license: cc-by-nc-sa`，即 CC BY-NC-SA 4.0），文章页脚将相应展示内容使用许可徽章；未声明的稿件不附带任何内容使用许可。
